...
export function describeArchiveSourceFinding(
	...
	if (facts.failedEvidenceRows > 0 || facts.scannerIssues > 0) {
		const retryDetails = [
			...
			.join(' and ');
		return {
			detail: `${formatFindingCount(facts.provenCheckpointProofs, 'root-checkpoint attestation')} verified across ${formatSourceCount(sourceCount)}; ${retryDetails} awaiting retry.`,
