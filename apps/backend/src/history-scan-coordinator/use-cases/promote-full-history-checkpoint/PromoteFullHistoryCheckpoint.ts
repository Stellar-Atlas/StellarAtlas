...
export class PromoteFullHistoryCheckpoint {
	constructor(
	...
	async promote(
		...
		if (firstLedger === undefined || lastLedger === undefined) {
			throw new FullHistoryPromotionError(
				...
				'Decoder returned no canonical ledger rows'
			);
		}

		return this.canonicalRepository.writeCheckpoint({
...
function assertCandidateMatchesTarget(
	...
	if (
		...
		throw new FullHistoryPromotionError(
			'invalid-proof',
			'Loaded checkpoint proof does not match the promotion target'
		);
	}
	if (candidate.proof.networkPassphrase !== target.networkPassphrase) {
		throw new FullHistoryPromotionError(
			'invalid-network-passphrase',
			'Loaded checkpoint proof does not match the target network passphrase'
		);
	}
}
