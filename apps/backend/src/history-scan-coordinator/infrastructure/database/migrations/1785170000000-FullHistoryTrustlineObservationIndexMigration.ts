...
const indexName = 'idx_full_history_lcm_trustline_observation';
...
export const fullHistoryTrustlineObservationIndexSql = `
	create index concurrently if not exists
		"idx_full_history_lcm_trustline_observation"
...
export class FullHistoryTrustlineObservationIndexMigration1785170000000 implements MigrationInterface {
	readonly name = 'FullHistoryTrustlineObservationIndexMigration1785170000000';
	...
	async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(fullHistoryTrustlineObservationIndexSql);
	}
...
async function readIndexState(
	...
	if (!Array.isArray(result)) {
		throw new Error(
			'Trustline observation index state query did not return rows'
		);
	}
	...
	if (!isIndexStateRow(row)) {
		throw new Error(
			'Trustline observation index state query returned invalid data'
		);
	}
	return row;
}
