...
export async function getHistoryArchiveTransitionReconciliation(
	...
	const [row] = (await manager.query(reconciliationHealthSql)) as readonly ReconciliationRow[];
	const pendingTerminalEffects = requireNumber(
		row?.pendingTerminalEffects ?? row?.pendingterminaleffects ?? 0,
		'pendingTerminalEffects'
	);
	const rawOldestPendingAt = row?.oldestPendingAt ?? row?.oldestpendingat ?? null;
...
const reconciliationHealthSql = `
	...
	where object."transitionEffectsRequiredAt" is not null
