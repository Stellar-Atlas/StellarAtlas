...
export const NetworkV1Schema: JSONSchemaType<NetworkV1> = {
	...
	properties: {
		...
		transitiveQuorumSet: {
			...
			type: 'array',
			description:
				'array of nodes in the transitive quorum-set of the trust graph with no outgoing edges'
		},
		scc: {
			items: {
				type: 'array',
		...
		stellarCoreVersion: {
			type: 'string',
			description:
				'Latest Stellar core version running on the nodes of the network',
			nullable: true
		},
		quorumSetConfiguration: { $ref: '#/definitions/BaseQuorumSet' }
	},
	type: 'object',
	required: [
		'nodes',
		...
		NetworkStatisticsV1: {
			properties: {
				...
				minBlockingSetFilteredSize: {
					...
					type: 'number',
					description:
						'The size of the smallest network blocking set with failing nodes filtered out'
				},
				minBlockingSetOrgsFilteredSize: {
					type: 'number',
					description:
						'The size of the smallest network blocking set grouped by organizations, with failing organizations filtered out'
				},
				minBlockingSetOrgsSize: {
					type: 'number',
					description:
						'The size of the smallest network blocking set grouped by organizations'
				},
				minBlockingSetSize: {
					type: 'number',
				...
				minSplittingSetCountrySize: {
					type: 'number',
					description:
						'The size of the smallest network splitting set, grouped by countries'
				},
				minSplittingSetISPSize: {
					type: 'number',
					description:
						'The size of the smallest network splitting set, grouped by ISPs'
				},
				minSplittingSetSize: {
					type: 'number',
