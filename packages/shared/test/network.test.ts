...
test('fromJSON', () => {
	...
	expect(
		network.quorumSetConfiguration?.innerQuorumSets[0].validators.length
	).toBe(1);
	expect(network.quorumSetConfiguration?.innerQuorumSets[0].validators[0]).toBe(
		'a'
	);
