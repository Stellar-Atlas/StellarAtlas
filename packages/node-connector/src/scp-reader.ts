...
export class ScpReader {
	...
	private isNewNominateVote(
		...
		if (!ledgerVotes) return true;

		const votesByNode = ledgerVotes.get(publicKey);
		if (!votesByNode) return true;
	...
	private isNewNominateAccepted(
		...
		if (!ledgerAccepted) return true;

		const acceptedByNode = ledgerAccepted.get(publicKey);
		if (!acceptedByNode) return true;
	...
	private translateCommit(
		...
		console.log(
			ledger + ': ' + name + ':VOTE|ACCEPT(PREPARE<Inf,' + ballotValue + '>)'
		);
		console.log(
			ledger +
				': ' +
	...
	private translateExternalize(
		...
		console.log(
			ledger + ': ' + name + ':ACCEPT(PREPARE<Inf,' + ballotValue + '>)'
		);
		console.log(
			ledger +
				': ' +
	...
	private translateNominate(
		...
		if (this.isNewNominateVote(ledger, publicKey, nominateVotes)) {
			console.log(
				ledger + ': ' + name + ':VOTE(NOMINATE([' + nominateVotes + ']))'
			);
